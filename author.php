				...
				<header class="page-header author-header">
					...
					if ( ! empty( $curauth->ID ) ) {
						$alt = sprintf(
							...
							_x( 'Profile picture of %s', 'Avatar alt', 'tracomme2023' ),
					...
					if ( ! empty( $curauth->user_url ) || ! empty( $curauth->user_description ) ) {
						...
						<dl>
							<?php if ( ! empty( $curauth->user_url ) ) : ?>
								<dt><?php esc_html_e( 'Website', 'tracomme2023' ); ?></dt>
								<dd>
							...
							<?php if ( ! empty( $curauth->user_description ) ) : ?>
								<dt>
									...
									printf(
										/* translators: %s: author name */
										esc_html__( 'About %s', 'tracomme2023' ),
										$curauth->display_name
									);
									?>
								</dt>
					...
					if ( have_posts() ) {
						printf(
							/* translators: %s: author name */
							'<h2>' . esc_html__( 'Posts by %s', 'tracomme2023' ) . '</h2>',
							$curauth->display_name
						);
					}
					?>
